...
| 无障碍专项检查 | `/accessibility` | 扶手/坡道/盲道/专用间四项设施配置与完好状态登记、按检查标准自动判定符合/部分符合/不符合、不符合项自动生成待整改事项、区域与公厕两级达标率汇总 |
...
│   │   ├── core                 # 配置、数据库、业务常量（含无障碍判定标准）、领域异常
│   │   ├── models               # ORM 模型：公厕、巡查、无障碍检查、问题、整改流水
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、无障碍判定与汇总、问题整改、评分、统计
│   │   ├── seed.py              # 演示数据生成
...
| GET | `/accessibility-checks` | 无障碍专项检查查询（restroom_id/district/result/inspector/日期区间/关键字） |
| POST | `/accessibility-checks` | 新增专项检查，自动判定四项设施并就不符合项生成待整改事项 |
| GET/DELETE | `/accessibility-checks/{id}` | 详情（含生成的整改事项）/ 删除（整改事项保留并解除关联） |
| GET | `/accessibility-checks/summary` | 无障碍达标率汇总（总览 + 区域 + 公厕） |
...
- **无障碍判定**：必检扶手、坡道、盲道、专用间四项。单项判定：未配置或严重损坏 → 不符合，轻微破损 → 部分符合，完好 → 符合。整体结论：任一不符合 → 不达标；无不符合但有部分符合 → 部分达标；全部符合 → 达标。达标率 =（符合×1 + 部分符合×0.5）/ 4 × 100。
- **自动建单**：专项检查中每项不符合设施自动生成一条待整改问题（分类「设施损坏」、期限 7 天、标题形如「无障碍坡道未配置」），并回链来源检查记录；同一公厕同一设施已存在未闭环事项时不重复建单，闭环后再次检出会重新建单。
- **达标率汇总**：公厕取最近一次专项检查结论与达标率；区域达标率 = 达标公厕数 / 区域公厕总数 × 100，同时统计未检查公厕数与待整改事项数。
...
`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、17 次左右无障碍专项检查（含四项设施登记与判定，不符合项自动生成整改事项并模拟部分流转）、一批不同整改阶段的问题及其完整整改轨迹。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。

## 测试与验证

```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、无障碍判定与汇总、流程流转、统计）
cd frontend && npm run build     # 生产构建
```

本项目完成时已实际运行验证：

- 后端 `pytest`：11 个用例全部通过；`/health`、台账/巡查/无障碍检查/问题/统计/字典接口均返回预期数据。
