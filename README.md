...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 无障碍专项检查 → 问题上报 → 整改跟踪** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。
...
| 无障碍检查 | `/accessibility` | 扶手/坡道/盲道/专用间配置与完好状态登记、按检查标准自动判定符合性、不符合项自动生成待整改事项、按区域与公厕汇总达标率 |
| 问题上报 | `/issues`、`/issues/:id` | 问题上报（可关联巡查记录）、分类/程度/期限、整改流程流转、整改轨迹时间线、超期预警、追加跟进记录 |

其他页面不会互相混杂：台账、巡查、无障碍检查、问题各自独立成页，详情页再做跨模块的关联展示。
...
├── backend
│   ├── app
│   │   ├── api/v1/endpoints      # 路由层：restrooms / inspections / accessibility / issues / stats / meta
│   │   ├── core                 # 配置、数据库、业务常量、领域异常
│   │   ├── models               # ORM 模型：公厕、巡查、无障碍检查、问题、整改流水
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、无障碍判定、问题整改、评分、统计
│   │   ├── seed.py              # 演示数据生成
...
│   │   ├── pages                # dashboard / restrooms / inspections / accessibility / issues 五个模块
│   │   ├── utils                # 时间格式化、评分换算
│   │   └── styles/global.css
│   ├── nginx.conf
│   └── Dockerfile
...
| GET | `/accessibility-inspections` | 无障碍检查记录查询（restroom_id/district/inspector/conformity/日期区间/关键字） |
| POST | `/accessibility-inspections` | 登记无障碍检查，服务端按检查标准判定符合性，不符合项自动生成待整改事项 |
| GET | `/accessibility-inspections/summary` | 无障碍达标率汇总（总体 + 按区域 + 按公厕） |
| GET/PATCH/DELETE | `/accessibility-inspections/{id}` | 详情 / 更新 / 删除 |
...
- **无障碍专项检查**：固定登记 4 项设施（无障碍扶手、无障碍坡道、盲道、无障碍专用间）的配置情况与完好状态，服务端按检查标准判定：未配置 → 不符合；已配置且完好 → 符合；轻微破损 → 部分符合；严重损坏 → 不符合。总体判定取各单项中最差的一档（存在不符合即总体不符合，否则存在部分符合即总体部分符合）。达标率 =（符合项 × 1 + 部分符合项 × 0.5）/ 检查项数 × 100。
- **待整改事项自动生成**：无障碍检查中每个不符合项自动生成一条「待整改」问题工单（分类为「无障碍设施」，未配置 → 严重、3 天整改期，严重损坏 → 一般、7 天整改期），并回链到本次检查；同一公厕同一设施已有未闭环工单时不重复生成。工单与手工上报的问题走同一套整改闭环流程。
- **达标率汇总**：按公厕汇总检查次数、最近一次判定与达标率、历次平均达标率、未闭环整改数；按区域汇总时，区域达标率取区域内各公厕最近一次检查达标率的平均，未检查的公厕不计入。
...
- **删除保护**：删除公厕时若已存在巡查、无障碍检查或问题记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、14 条无障碍专项检查记录（不符合项自动生成「无障碍设施」类整改工单）、30 余条不同整改阶段的问题及其完整整改轨迹。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。

> 从旧版本升级时，SQLite 库会在启动时自动为 `issues` 表补充 `accessibility_inspection_id` 列，无需手工迁移。

## 测试与验证

```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、无障碍判定与自动工单、达标率汇总、流程流转、统计）
cd frontend && npm run build     # 生产构建
```

本项目完成时已实际运行验证：

- 后端 `pytest`：13 个用例全部通过；`/health`、台账/巡查/无障碍检查/问题/统计/字典接口均返回预期数据。
